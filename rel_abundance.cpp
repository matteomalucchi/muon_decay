void rel_abundance(){
    double n_pos [] = {275, 316, 103, 107, 165, 190, 224, 303};
    double n_neg [] = {194, 1165, 61, 39, 310, 164, 1057, 463};
    double n_pos_err [] = {8, 9, 17,18, 25, 28, 9, 10};
    double n_neg_err [] = {58, 1013, 13, 9, 71, 22, 167, 66};
    const int dimension = sizeof(n_pos)/sizeof(n_pos[0]);
    double ab [dimension] = {};
    double ab_err [dimension] = {};


    for (int i=0; i < dimension; i++){
        ab[i]=n_pos[i]/n_neg[i];
        ab_err[i] = sqrt(pow(n_pos_err[i]/n_neg[i],2)+pow(n_pos[i]*n_neg_err[i]/(n_neg[i]*n_neg[i]),2));
        cout << ab[i] << " +- " << ab_err[i] <<endl;
    }
